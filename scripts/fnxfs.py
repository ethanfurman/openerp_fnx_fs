...
if __name__ == "__main__":
    print repr(argv), len(argv)
    if len(argv) > 1:
        print 'usage: %s' % argv[0]
        exit(1)
    user = get_login()
    fuse = FUSE(FnxFS(host=openerp, server_user=server_user, server_pass=server_pass, user=user),
            client_root/user/'fnx_fs', foreground=True, nothreads=True)
